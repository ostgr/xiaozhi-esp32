...
void McpServer::AddCommonTools()
{
	...
	if (display && display->GetTheme() != nullptr)
	{
		...
		AddTool("self.screen.set_rotation",
				...
				PropertyList({Property("rotation_degree", kPropertyTypeInteger, 0, 270)}),
				[display](const PropertyList &properties) -> ReturnValue
				{
	...
	if (music)
	{
		AddTool(
			...
			"IMPORTANT: Do NOT read the song_id to the user. Just say you are playing the song.\n"
			"Parameters:\n"
	...
	if (sd_music)
	{
		...
		AddTool(
			...
			[sd_music](const PropertyList &) -> ReturnValue
			{
				...
				switch (state)
				{
				case Esp32SdMusic::PlayerState::Stopped:
					s = "stopped";
					break;
				case Esp32SdMusic::PlayerState::Preparing:
					s = "preparing";
					break;
				case Esp32SdMusic::PlayerState::Playing:
					s = "playing";
					break;
				case Esp32SdMusic::PlayerState::Paused:
					s = "paused";
					break;
				case Esp32SdMusic::PlayerState::Error:
					s = "error";
					break;
				}
