...
void Application::Start()
{
    ...
    xTaskCreate([](void *arg)
        ...
        vTaskDelete(NULL); }, "main_event_loop", 1024 * 3, this, 3, &main_event_loop_task_handle_);
...
void Application::SetDeviceState(DeviceState state)
{
    ...
    led->OnStateChanged();
    // Stop music playback when transitioning from idle or streaming state to any other state
    if ((previous_state == kDeviceStateIdle || previous_state == kDeviceStateStreaming) && state != kDeviceStateStreaming)
    {
        if (music_)
        {
...
void Application::AddAudioData(AudioStreamPacket &&packet)
{
    ...
    if ((device_state_ == kDeviceStateIdle || device_state_ == kDeviceStateStreaming))
    {
